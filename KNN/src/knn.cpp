  ...
  for (int i = 0; i < m_k; ++i) {
    if (i == 0) {  // for the first value
      for (auto& feature : *m_trainingData) {  // check each feature in m_trainingData
        ...
        if (distance < minDist) {  // check if this is the minimum distance
          minDist = distance;  // store the minimum distance
          closestPtr = feature;  // store the closest feature
        }
      }
      ...
      for (auto& feature : *m_trainingData) {  // check each feature in m_trainingData
        ...
        // only update closestPtr if this is farther than the previous closest
        if (distance > previousMinDist && distance < minDist) {
          minDist = distance;  // store the minimum distance
          closestPtr = feature;  // store the closest feature
        }
      }
    }
    m_neighbors.push_back(closestPtr);  // append the closest feature
    previousMinDist = minDist;  // keep track of the previous min
    minDist = std::numeric_limits<double>::max();  // reset the min value
  }
}
